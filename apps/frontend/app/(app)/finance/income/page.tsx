...
export default function IncomePage() {
  ...
  const { data } = useQuery<IncomeItem[]>({ queryKey: ['income'], queryFn: () => apiClient.get('/income') });
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      setForm({ date: new Date().toISOString().slice(0, 10), categoryId: '', description: '', amount: '', paymentMethod: 'CASH' });
    },
    // Distingue el bloqueo de período cerrado (PRD sección 15) de cualquier
    // otro error de validación, para mostrar un mensaje claro al usuario.
        ...
        <tbody className="divide-y">
          {data?.map((i) => (
            <tr key={i.id}>
              ...
              <td className="p-3">{currency(i.amount)}</td>
            </tr>
          ))}
        </tbody>
      </table>
